...
export default function BusinessTab() {
  ...
  const onSubmit = async (values: BusinessForm) => {
    await updateWorkspace({
      ...
      ibanNumber:        bankFields === 'iban'    ? (values.ibanNumber    || null) : null,
      swiftCode:         bankFields === 'iban'    ? (values.swiftCode     || null) : null,
      routingNumber:     bankFields === 'routing' ? (values.routingNumber || null) : null,
    })
    setSaved(true)
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="card-glass p-6 space-y-5">
        ...
        {!isIndia && (
          <Field label="Tax Label" hint="Shown on invoices — e.g. VAT, Sales Tax, GST">
            <input
              {...register('taxLabel')}

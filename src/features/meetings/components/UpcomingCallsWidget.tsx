...
function timeChip(iso: string): { label: string; chipClass: string } {
  ...
  if (minsAway <= 30)  return { label, chipClass: 'bg-[#FEF3F2] text-[#D92D20]' }
  if (minsAway <= 120) return { label, chipClass: 'bg-[#FFFAEB] text-[#B54708]' }
  return                      { label, chipClass: 'bg-[#ECFDF3] text-[#027A48]' }
}
...
function MeetingRow({ meeting }: { meeting: Meeting }) {
  ...
  return (
    <div className="flex items-center gap-3 px-5 py-3.5 hover:bg-[#FAFBFF] transition-colors">
      <div className="shrink-0 text-center min-w-[56px]">
        <p className="text-[10px] text-[#98A2B3] font-medium">{dateLabel(meeting.scheduledAt)}</p>
      ...
      <div className="flex-1 min-w-0">
        ...
        {contact && <p className="text-[11px] text-[#98A2B3] truncate">{contact}</p>}
      </div>
      {meeting.meetLink ? (
        <a
          ...
          className="shrink-0 flex items-center gap-1 px-2.5 py-1 rounded-lg bg-[#ECFDF3] text-[#027A48] text-[11px] font-semibold hover:bg-[#D1FAE5] transition-colors"
        ...
        <span className="shrink-0 text-[11px] text-[#D0D5DD] px-2">No link</span>
      )}
    </div>
  )
}
  ...
  return (
    <div className="card overflow-hidden h-full hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between px-5 py-4 border-b border-[#F2F4F7]">
        <div>
          <h2 className="text-[14px] font-bold text-[#101828]">Upcoming Calls</h2>
          <p className="text-[12px] text-[#98A2B3] mt-0.5">Scheduled this week</p>
        </div>
        <div className="flex items-center gap-2">
          {!isLoading && meetings.length > 0 && (
            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold bg-[#ECFDF3] text-[#027A48]">
              {meetings.length} upcoming
            </span>
          )}
          <div className="w-8 h-8 rounded-xl bg-[#ECFDF3] flex items-center justify-center">
            <CalendarDays size={14} className="text-[#027A48]" strokeWidth={2} />
          </div>
        </div>
      </div>
      ...
      <div className="divide-y divide-[#F2F4F7]">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-8">
            <CalendarDays size={26} className="text-[#D0D5DD] mb-2" strokeWidth={1.5} />
            <p className="text-[13px] text-[#98A2B3]">No calls scheduled this week</p>
            <button

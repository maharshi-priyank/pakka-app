...
import { FileArchive, FileText, FileImage, File, Trash2, Upload, Lock, Loader2 } from 'lucide-react'
...
export default function InvoiceFilesPanel({ invoice }: Props) {
  const inputRef = useRef<HTMLInputElement>(null)
  const [showCanvaPicker, setShowCanvaPicker] = useState(false)
  const parent = { invoiceId: invoice.id }

  const { data: attachments = [], isLoading } = useAttachments(parent)
  const uploadMutation    = useUploadAttachment(parent, { gateInvoiceId: invoice.id })

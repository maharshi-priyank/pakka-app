  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-extrabold text-[#101828] tracking-tight">Clients</h1>
...
function ClientCard({ client, onClick }: { client: Client; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="card p-5 text-left hover:shadow-md hover:border-[#D0D5DD] transition-all group"
    >
      <div className="flex items-start gap-3 mb-4">
        <div className="w-10 h-10 rounded-xl bg-[#EFF6FF] flex items-center justify-center shrink-0 border border-[#DBEAFE]">
          <span className="text-[13px] font-bold text-[#2563EB]">{initials}</span>
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-[13.5px] font-bold text-[#101828] truncate group-hover:text-[#2563EB] transition-colors">
            {client.name}
          </p>
          {client.company && (
            <p className="text-[12px] text-[#667085] truncate">{client.company}</p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-1.5 mb-4">
        {client.email && (
          <div className="flex items-center gap-2">
            <Mail size={11} className="text-[#98A2B3] shrink-0" />
            <span className="text-[12px] text-[#667085] truncate">{client.email}</span>
          </div>
        )}
        {client.phone && (
          <div className="flex items-center gap-2">
            <Phone size={11} className="text-[#98A2B3] shrink-0" />
            <span className="text-[12px] text-[#667085]">{client.phone}</span>
          </div>
        )}
        {!client.email && !client.phone && (
          <p className="text-[12px] text-[#D0D5DD]">No contact info</p>
        )}
      </div>
      ...
      <p className="text-[11px] text-[#D0D5DD] mt-3">Added {formatDate(client.createdAt)}</p>
    </button>
  )
}
...
function CountPill({
  ...
  return (
    <div className="flex items-center gap-1">
      <Icon size={10} className="text-[#98A2B3]" />
      <span className="text-[11.5px] font-semibold text-[#667085]">{count}</span>
...
function EmptyState({ search, onAdd }: { search: string; onAdd: () => void }) {
  if (search) {
    return (
      <div className="card p-10 text-center">
        <Search size={32} className="text-[#D0D5DD] mx-auto mb-3" />
        <p className="text-[14px] font-semibold text-[#101828]">No clients found</p>
        <p className="text-[13px] text-[#98A2B3] mt-1">No results for "{search}". Try a different search.</p>
      </div>
    )
  }
  ...
  return (
    <div className="card p-10 text-center">
      ...
      <p className="text-[15px] font-bold text-[#101828]">No clients yet</p>
      <p className="text-[13px] text-[#98A2B3] mt-1 max-w-xs mx-auto">

...
- **Objective:** Build a common admin panel for the pakka system — a gated area inside pakka-app that lets internal staff see system-wide data and look up per-user / per-workspace data, and run operational actions. Embedded as a superadmin-only `/admin` route group in pakka-app, backed by a new dedicated admin API in the existing NestJS backend.
...
- R14. Staff can run billing/refund operations: issue a refund through the existing Cashfree or Stripe payment provider, re-sync a stuck subscription, and replay a billing event. No refund endpoint exists today, so the refund path is new admin-backed machinery built on the existing providers, not a call to an existing refund route. These touch real money and carry the strongest confirmation and audit requirements.
...
- **Embedded route in pakka-app over a sibling project** (session-settled: user-approved — chosen over a new `pakka-admin` sibling + vendored theme: gives "same design theme" at zero sharing cost, one deploy, reuses existing auth/Query/shell). Governs R1, R3.
...
- Re-architecting billing providers. The admin builds on the existing Cashfree + Stripe machinery; it does not replace it.
...
- The existing Cashfree + Stripe subscription endpoints and webhooks under `/payments/*` are reused for subscription ops. A refund capability does not exist today, so the admin introduces a new refund path built on the existing providers rather than wrapping an existing refund endpoint.
...
- KTD4. **Refund = new `refund` method on the `PaymentProvider` interface, implemented for Cashfree and Stripe, idempotent on paymentId.** No refund endpoint exists today. Add `refund(paymentId, amount?)` to `PaymentProvider`; Cashfree implements `POST /orders/{id}/refunds` with its `x-client-id/secret` headers, Stripe implements `stripe.refunds.create({ payment_intent, amount })`. An admin `POST /admin/billing/refund` calls the injected provider (same `PAYMENT_PROVIDER` token `PaymentsService` uses) and writes an audit entry. The refund endpoint is **idempotent on paymentId**: a second refund call for an already-refunded payment short-circuits (returns the existing refund result) rather than re-calling the provider — preventing a retried/replayed request or a double-click from issuing two real-money refunds. Each refund attempt writes an audited `BillingEvent`/`AuditLog` row keyed by the idempotency key so a retried call surfaces "already refunded." Provider-specific idempotency-key mechanics (Stripe `Idempotency-Key` header, Cashfree idempotency support) are wired into the `refund` signature; exact partial-vs-full amount handling per provider is deferred to implementation. Governs R14.
...
- KTD6. **Frontend admin = separate `AdminRoute` guard + own layout, reusing theme primitives, ConfirmModal, recharts, and the axios client.** A new `src/features/admin/` feature module mirrors the existing domain convention. Admin auth uses a separate Zustand store + axios instance hitting `/admin/auth/*`; the `/admin/*` route group is a new top-level child in `createBrowserRouter` guarded by `AdminRoute` (checks the admin session), rendering a dedicated admin layout (not `AppShell`/`WorkspaceProvider`, which assume a tenant workspace). Currency in admin views comes from each target record's own `currency` field, never `useWorkspace()`. Governs R1, R8–R10.
...
- **Files:**
  ...
  - `pakka-backend/src/modules/payments/cashfree.provider.ts` — implement `refund` via `POST /orders/{id}/refunds`.
...
- **Approach:**
  1. Add `refund` to the interface; implement on both providers (Cashfree `POST /orders/{id}/refunds` with `x-client-id/secret` headers; Stripe `refunds.create({ payment_intent, amount })`). Per KTD4.
...
- **Patterns to follow:** `cashfree.provider.ts:1-30` (provider method + `headers` pattern), `stripe.service.ts:1-45` (`this.stripe` SDK pattern), `payments.service.ts:1-33` (`@Inject(PAYMENT_PROVIDER)` injection).
- **Test scenarios:**
  - `provider.refund` on Cashfree posts to `/orders/{id}/refunds` with correct headers (mock fetch); on Stripe calls `refunds.create` with the payment intent (mock SDK).
